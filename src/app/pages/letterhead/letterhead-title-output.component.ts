...
@Component({
  ...
  template: `<p *ngIf="titleLine.include" class="title"><img class="text-icon" [ngClass]="{'none': !titleLine.showIcon}"
    ...
    <span class="ml-6 {{titleLine.part1.class}}">{{titleLine.part1.text}}</span>
    <span class="ml-6 {{titleLine.part2.class}}">{{titleLine.part2.text}}</span>
    <span class="ml-6 {{titleLine.part3.class}}">{{titleLine.part3.text}}</span></p>
  `
})
