...
import { MailchimpApiError, MailchimpErrorResponse, MailchimpHttpErrorResponse } from "../../../../projects/ngx-ramblers/src/app/models/mailchimp.model";
...
export function campaignReplicate(req: Request, res: Response): Promise<void> {
  return configuredMailchimp().then(config => {
    return config.mailchimp.campaigns.replicate(req.params.campaignId)
      .then(responseData => {
